...
local hengyang_table_creator = require "hengyang.table_frame"
local hengyang_game_creator = require "hengyang.game_sink"
...
function command.INIT(args)
    ...
    --2长沙 1湖南 7郴州 8红中（用转转的game_sink） 10宁乡麻将 28常德麻将 81衡阳麻将
    ...
    elseif args.game_id == 28 then
        ...
        table_sink:set_game_sink(game_sink)
    elseif args.game_id == 81 then
        table_sink = hengyang_table_creator.new(args)
        table_interface = table_sink:get_interface()
        game_sink = hengyang_game_creator.new(table_interface)
        table_sink:set_game_sink(game_sink)
	else
