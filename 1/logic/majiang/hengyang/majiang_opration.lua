require "table_util"
local hupai = require "majiang.hengyang.hupai"
...
--    end
    ...
    local fan_tmp = get_fan_point(hu_ret)
    louHuChair[chair_id] = louHuChair[chair_id] or 0
    if next(hu_ret) and fan_tmp > louHuChair[chair_id] then
		ret.canHu = true
        ret.hutype = 1
        ret.hucard = card
        hu_ret.hucard = card
...
--isnext 是不是下家
function majiang_operation:other_out_card(cardInfo, otherCard, chair_id, louHuChair, isfirst, isnext, lock, isgang, cardnum, loupeng)
	local ret = {}
    ...
    if not lock then
	    ...
	    if canPeng(cardInfo.stackCards, otherCard, self.table_config) and (not loupeng or not loupeng[otherCard]) then
...
function majiang_operation:check_ting_card(testCardTmp, cardInfo, config)
    return hupai:check_can_ting(testCardTmp, cardInfo, config)
...
function majiang_operation:other_self_bu(cardInfo, otherCard, chair_id, louHuChair, isgang)
	local ret = {}
	local tmpCard = table.clone(cardInfo.handCards)
	table.insert(tmpCard, otherCard)
    local config = {}
