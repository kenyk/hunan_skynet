local cardDef = require "majiang.hengyang.cardDef"
...
--碰碰胡
local function testPengPengHu(cardInfo, stackCards, ischeckting)
...
--判断是否都是同一类型的牌
local function testSingcolor(cardInfo, stackCards, card)
    local type = 0
    for i, k in pairs(stackCards) do
        if type == 0 then 
            type = getPaiType(i)
...
-- 海底捞月  海底炮 
--检测门清 
local function check_menqing(card_info)
    if next(card_info.chiCards) then
        return false
    end
    if next(card_info.pengCards) then
        return false
    end
    for i , k in pairs(card_info.gangCards) do
        if k[1] ~= 2 then
            return false
        end
    end
...
function hupai:check_can_hu(pai, cardInfo, config)
	local tmpcard = m_table.clone(pai)
	table.sort(tmpcard)
	--计算癞子数
	local curHunNum = 0
    ...
    --是否同一色（清一色）
    local issingleType = testSingcolor(cardInfo, tmpcardtable)
    --全求人
    local allAsk = false
    if #pai == 2 and check258(pai[1]) then
       allAsk = true 
    end
    --乱将
	...
	if isHu then
        ...
        if testPengPengHu(cardInfo, tmpcardtable) then
            ret.pengpenghu = 1
    ...
    if isHu then
        ...
        if not next(ret) then
            ...
            if check_menqing(cardInfo) then
                ret.menqing = 1
            end
        end
    end
...
function hupai:check_can_ting(pai, cardInfo, config, card)
	local tmpcard = m_table.clone(pai)
	table.sort(tmpcard)
	--计算癞子数
	local curHunNum = 1
	...
	local tmpcardtable = cardDef:stackCards(tmpcard)
    --是否同一色（清一色）
    local issingleType = testSingcolor(cardInfo, tmpcardtable, card)
    --全求人
    local allAsk = false
    if #pai == 1 then
       allAsk = true
...
--    else
        ...
        if testPengPengHu(cardInfo, tmpcardtable, true) then
            return true
        end
	    --判断正常胡牌
	    if testHuPai(tmpcard, 4, curHunNum, luanjiang) then
