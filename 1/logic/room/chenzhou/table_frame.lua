        ...
        if args.option == 0 then
            ...
            if  sign then
                ...
                if self.table_config.game_index == 1 then
                    ...
                    result, sclub = self.game_sink:get_all_balance_result()
                end
                skynet.send(".BUILD_ROOMS_SERVICE", "lua", "on_close_room",skynet.self(),{address = self.address, room_name = self.room_name, list = list , enter_code = self.enter_code, sclub = sclub})
...
function table_frame:dismiss(args)
    for k, v in pairs(self.lookon_players) do   
        if v.address  then
            ...
            datacenter.set("on_player", k, nil)
        end
    end
    for k, v in pairs(self.table_players) do
        if v.address then
...
function table_frame:on_game_end(args)
    ...
    if (self.table_config.game_index == self.table_config.game_count + 1)  or ( args )   then
    	...
    	ret.player_result, sclub= self.game_sink:get_all_balance_result()
        ret.win = sclub
    	--printT(ret)
