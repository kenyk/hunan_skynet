...
local majiang = require "majiang.chenzhou.cardDef"
local majiang_opration = require "majiang.chenzhou.majiang_opration"
local game_player_info = require "majiang.chenzhou.game_player_info"
...
--飘
function game_sink:piao_handler(chair_id, point)
    if not self.piao_point[chair_id] then
        self.piao_point[chair_id] = point
        self.piao_count = self.piao_count + 1
        self.game_record:record_game_action(chair_id, 9, point)
        if self.piao_count == #self.players then
            self:send_table_client(0, "game_piao_point", {info = self.piao_point})
            self:deal_cards()
...
function game_sink:peng_card(chair_id, card, iswait)
	...
	for k, v in pairs(self.players) do
		-- if k ~= chair_id then
			self:send_table_client(k, "game_peng_card", {chair_id = chair_id, card = card, out_card = self.game_all_info.last_out_chair})
		-- end
	end
...
--        if k ~= chair_id then
----            local ret = majiang_opration:other_out_card(v.card_info.handCards, v.card_info.stackCards,card, k, self.louHuChair)
--            local ret = majiang_opration:other_self_gang(v.card_info.handCards, v.card_info.stackCards,card, k, {})
--            if next(ret) then
--			    self:count_can_operation(ret, k, 2, chair_id,card)
--			    will_wait = true
--			    --TODO:推送给玩家可以进行的操作
...
function game_sink:gang_mo_card(chair_id, card)
	...
	for k, v in pairs(self.players) do
		if true then
			...
			self:send_table_client(k, "game_gang_card", param)
            if k ~= chair_id and ret.gang_type == 1 and self.game_config.game_type == 1 then
...
function game_sink:deal_hu_balance(win_chair, lose_chair, pbirdPoint, op)
	...
	local birdPoint = 0
    --飘
    local win_piao = self.piao_point[win_chair] or 0
    local lose_piao = 0
	--自摸
	if lose_chair == 0 then
		for k, v in pairs(self.players) do
			if k ~= win_chair then
                lose_piao = self.piao_point[k] or 0
				if (win_chair == self.banker or k == self.banker) and self.game_config.idle then
					v.balance_info.huPoint = - (2 + 1) - win_piao - lose_piao
				else
					v.balance_info.huPoint = - 2 - win_piao - lose_piao
				end
	...
	elseif lose_chair ~= 0 then
		--接炮
        lose_piao = self.piao_point[lose_chair] or 0
		if op == 2 then
			local lose_point = 0
			if (self.banker == win_chair or self.banker == lose_chair) and self.game_config.idle then
				lose_point = -2 -lose_piao - win_piao 
				win_point = 2 + lose_piao + win_piao 
			else
				lose_point = -1 -lose_piao - win_piao 
				win_point = 1 + lose_piao + win_piao 
			end
			local game_balance_info = self.players[lose_chair].balance_info
			game_balance_info.huPoint = lose_point + game_balance_info.huPoint
		...
		elseif op == 3 then
			for k, v in pairs(self.players) do
				if k ~= win_chair then
                    lose_piao = self.piao_point[k] or 0
					if (win_chair == self.banker or k == self.banker) and self.game_config.idle then
						win_point = win_point - 3 - lose_piao - win_piao
					else
						win_point = win_point - 2 - lose_piao - win_piao
					end
					birdPoint = birdPoint + pbirdPoint
				end
			end
			local game_balance_info = self.players[lose_chair].balance_info
			game_balance_info.huPoint = win_point + game_balance_info.huPoint
			game_balance_info.birdPoint = -birdPoint + game_balance_info.birdPoint
...
function game_sink:hu_card(chair_id, card)
	...
	-- table.printT(self.game_privite_info.canHu)
	local huIndex = 0
    local hutype = 0
	...
	for k, v in pairs(self.game_privite_info.canHu) do
		...
		if v.chair_id == chair_id then
			...
			if not next(self.game_end_balance_info.birdCard) then
				...
				-- table.insert(self.players[chair_id].balance_info, self.game_end_balance_info.birdCard)
			end
			-- {chair_id = chair_id, op = op, lose_chair = lose_chair}
...
function game_sink:start_game()
    ...
    if not self.game_config.piao then
	    self:deal_cards()
	    self:draw_card(self:get_banker_chair(), 1)
        self:send_table_client(0, "game_start_game", {time = os.time()})
    else
        self:send_table_client(0, "game_start_game", {time = os.time(), needWait = true})
    end
end
...
--    end
	...
	for k, v in pairs(self.players) do
        ...
        tmp.piaoPoint = self.piao_point[k]
--		self:add_balance_point(k, tmp.huPoint + tmp.birdPoint + tmp.gangPoint)
...
function game_sink:post_game_reconnect(chair_id)
    ...
    if self.game_config.piao and self.piao_count < #self.players then
        local point = self.piao_point[chair_id] or -1 --   -1代表他还没选 game_reconnect_piao
        self:send_table_client(chair_id, "game_reconnect_piao", {point = point})
        return true, {code = 1} 
    end
	ret.code = 0
	ret.playerCard = {}
