...
local majiang = require "majiang.changsha.cardDef"
local majiang_opration = require "majiang.changsha.majiang_opration"
local game_player_info = require "majiang.changsha.game_player_info"
...
function game_sink:operationHandler(chair_id, card, card1, istipsClent)
    ...
    local ret, hu_ret = majiang_opration:mo_card(user_card_info, card, chair_id, self.louHuChair, self.isfirstOut, self.gangLock[chair_id],nil, card1, #self.aftercards)
    ...
    if card1 then
        ret1, hu_ret1 = majiang_opration:mo_card(user_card_info, card1, chair_id, self.louHuChair, self.isfirstOut, self.gangLock[chair_id], nil, card1, #self.aftercards)
	...
	if next(ret) then
        ...
        elseif ret1 and next(ret1) then   
            syslog.debug("玩家:["..chair_id.."]可第二次操作:xxxxx"..json.encode(ret))
...
--    end
    ...
    --			local ret = majiang_opration:other_out_card(v.card_info.handCards, v.card_info.stackCards,card, k, self.louHuChair)
                local ret, hu_ret = majiang_opration:other_out_card(v.card_info, card, k, self.louHuChair, self.isfirstOut, self:get_next_chair(chair_id)== k, self.gangLock[k], nil,#self.aftercards)
                if not self.game_privite_info.Hu_info then
                    self.game_privite_info.Hu_info = {}
                end
        ...
        for k, v in pairs(self.players) do
            if k ~= chair_id then 
                table.printForMajiang(k, v.card_info.handCards)
                local ret, hu_ret = majiang_opration:other_out_card(v.card_info, card, k, self.louHuChair, self.isfirstOut, self:get_next_chair(chair_id)== k, self.gangLock[k], card1, #self.aftercards)
                local ret1, hu_ret1 = majiang_opration:other_out_card(v.card_info, card1, k, self.louHuChair, self.isfirstOut, self:get_next_chair(chair_id)== k, self.gangLock[k], card1, #self.aftercards)                
                if not self.game_privite_info.Hu_info then
                    self.game_privite_info.Hu_info = {}
                end
                self.game_privite_info.Hu_info[k] = hu_ret
...
--            local ret = majiang_opration:other_self_gang(v.card_info, card, k, self.louHuChair)
--            if next(ret) then
--			    self:count_can_operation(ret, k, 2, chair_id,card, ret.hucard)
--			    will_wait = true
--			    --TODO:推送给玩家可以进行的操作
...
function game_sink:bu_peng_card(chair_id, card, iscallback)
	...
	local user_card_info = self.players[chair_id].card_info
	majiang_opration:handle_gang_peng_card(user_card_info.handCards, user_card_info.stackCards, user_card_info.gangCards, card)
	--把牌从出过的牌中拿走
	local out_user_card_info = self.players[self.game_all_info.last_out_chair].card_info
	majiang_opration:handle_kick_out_card(out_user_card_info.outCards, card)
	...
	for k, v in pairs(self.players) do
		if true then
			local param = {chair_id = chair_id, card = card, bu_type = 3}
...
function game_sink:bu_mo_card(chair_id, card)
	...
	local ret = majiang_opration:handle_gang_mo_card(user_card_info.handCards, user_card_info.stackCards, user_card_info.pengCards, user_card_info.gangCards, card,chair_id)
	for k, v in pairs(self.players) do
		local param = {chair_id = chair_id, card = card, bu_type = ret.gang_type}
		self:send_table_client(k, "game_bu_card", param)
        if k ~= chair_id and ret.gang_type == 1 then
...
function game_sink:deal_hu_card(hu_type, info, chair_id, chair_id1)
    ...
    elseif hu_type == 2 then
        ...
        if chair_id1 then
            ...
            local bank_point = 0
            if self.game_config.idle and (self.banker == chair_id1 or self.banker == chair_id) then
                bank_point = 1
            end
            ...
            for i , k in pairs(self.players) do
                if i ~= chair_id then 
                    if self.game_config.idle and (self.banker == i or self.banker == chair_id) then
	...
	for k, v in pairs(self.game_privite_info.canHu) do
		...
		if v.chair_id == chair_id then
            if card == v.card then
                ...
                local huinfo 
                if self.game_privite_info.Hu_info[chair_id].hucard == card then
...
--    end
    ...
    for k , v in pairs(self.players) do 
        ...
        if v.changsha_balance_info.hu_info ~= "" then
