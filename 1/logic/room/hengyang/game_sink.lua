...
local majiang = require "majiang.hengyang.cardDef"
local majiang_opration = require "majiang.hengyang.majiang_opration"
local game_player_info = require "majiang.hengyang.game_player_info"
...
function game_sink:init_game()
    ...
    self.haidiCount = 0
    self.onHaidi = 0
    self.haidiType = 0 --海底进度 （1询问， 2有操作的时候）
	--游戏私有信息
    ...
    self.game_privite_info.Hu_info1 = {}
	self.game_privite_info.canPeng1 = 0
    self.game_privite_info.pengcard1 = 0
	self.game_privite_info.canGang1 = {chair_id = 0}
    self.game_privite_info.canBu1 = {chair_id = 0}
	self.game_privite_info.canChi1 = 0
    self.game_privite_info.Chi_info1 = {}
...
function game_sink:close_sink()
    ...
    self.onHaidi = nil
    self.haidiType = nil
    self.peng_card_turn = nil
    self.louPengChair = nil
end
...
--	if ret.canHu and not (self.louHuChair[chair_id] and self.louHuChair[chair_id][card]) then
	...
	if ret.canPeng then
        if self.game_privite_info.canPeng == 0 then
		    self.game_privite_info.canPeng = chair_id
            self.game_privite_info.pengcard = card
        else
		    self.game_privite_info.canPeng1 = chair_id
            self.game_privite_info.pengcard1 = card
        end
	end
	if ret.canGang then
		local tmp = {}
        ...
        if self.game_privite_info.canGang.chair_id == 0 then
		    self.game_privite_info.canGang = tmp
        else
            self.game_privite_info.canGang1 = tmp
        end
	end
    if ret.canBu then
		local tmp = {}
		tmp.chair_id = chair_id
		tmp.card = ret.canBu
        if self.game_privite_info.canBu.chair_id == 0 then
		    self.game_privite_info.canBu = tmp
        else
            self.game_privite_info.canBu1 = tmp
        end
    end
	if ret.canChi then
        if self.game_privite_info.canChi == 0 then
		    self.game_privite_info.canChi = chair_id
            self.game_privite_info.Chi_info = ret.canChi
        else
		    self.game_privite_info.canChi1 = chair_id
            self.game_privite_info.Chi_info1 = ret.canChi
        end
	end
--    table.printR(self.game_privite_info)
end
...
--备用存储
	if self.game_privite_info.canPeng1 == chair_id then
		self.game_privite_info.canPeng1 = 0
        self.game_privite_info.pengcard1 = 0
	end
    if self.game_privite_info.canBu1.chair_id == chair_id then
        self.game_privite_info.canBu1 = {chair_id = 0}
    end
	if self.game_privite_info.canGang1.chair_id == chair_id then
		self.game_privite_info.canGang1 = {chair_id = 0}
	end
	if self.game_privite_info.canChi1 == chair_id then
		self.game_privite_info.canChi1 = 0
        self.game_privite_info.Chi_info1 = {}
	end
    self.game_privite_info.Hu_info[chair_id] = nil
    self.game_privite_info.Hu_info1[chair_id] = nil

	for k, v in pairs(self.game_privite_info["canHu"]) do
		if v.chair_id == chair_id then
            self.game_privite_info["canHu"][k] = nil
		end
	end
end
...
--chair_id需要确认的人   old_chair_id是 已经存储 的人   out_chair_id 是出牌的人
function game_sink:check_pos(chair_id, old_chair_id, out_chair_id)
    local id = out_chair_id
    for i=1 , 4 do
        id = self:get_next_chair(id)
        if id == chair_id then
            return true
        elseif id == old_chair_id then
            return false
        end
    end
    return false
...
function game_sink:insert_player_operation(chair_id, option_type, card, gang_type)
	...
	if next(self.game_player_operation) then
		if player_op_priority[option_type] > self.game_player_operation.op then
            ...
            return true
        elseif player_op_priority[option_type] == self.game_player_operation.op then
            if self:check_pos(chair_id, self.game_player_operation.chair_id, self.game_all_info.last_out_chair) then
                self.game_player_operation = param
                return true
            else
                return false
...
function game_sink:deal_player_operation(chair_id)
	--处理海底胡取消
	if self.haidiType == 3 then
		self:game_end()
		return true
	elseif self.haidiType == 2 then
		self:deal_haidi_out_card(chair_id, self.game_all_info.last_out_card, true)
		return true
	end
	...
	elseif player_operation.op == 4 then
		flag = true
...
function game_sink:check_player_gang_need_wait(chair_id)
	...
	if self.game_privite_info["canGang"] ~= 0 and self.game_privite_info["canGang1"] ~= 0 and self.game_privite_info["canGang"] ~= self.game_privite_info["canGang1"] then
		local other_chair
		if self.game_privite_info["canGang"] == chair_id then
			other_chair = self.game_privite_info["canGang1"]
		else
			other_chair = self.game_privite_info["canGang"]
		end
		return self:check_pos(chair_id, other_chair, self.game_all_info.last_out_chair)
	end
	return true
end

function game_sink:check_player_bu_need_wait(chair_id)
	...
	if self.game_privite_info["canBu"] ~= 0 and self.game_privite_info["canBu1"] ~= 0 and self.game_privite_info["canBu"] ~= self.game_privite_info["canBu1"] then
		local other_chair
		if self.game_privite_info["canBu"] == chair_id then
			other_chair = self.game_privite_info["canBu1"]
		else
			other_chair = self.game_privite_info["canBu"]
		end
		return self:check_pos(chair_id, other_chair, self.game_all_info.last_out_chair)
	end
	return true
...
function game_sink:check_player_peng_need_wait(chair_id)
	...
	if not self:check_player_bu_need_wait(chair_id) then
		return false
	end
	if self.game_privite_info["canGang"].chair_id ~= 0 and self.game_privite_info["canGang"].chair_id ~= chair_id and self.game_privite_info["canGang1"].chair_id ~= chair_id then
		return false
	end
	if self.game_privite_info["canPeng"] ~= 0 and self.game_privite_info["canPeng1"] ~= 0 and self.game_privite_info["canPeng"] ~= self.game_privite_info["canPeng1"] then
		local other_chair
		if self.game_privite_info["canPeng"] == chair_id then
			other_chair = self.game_privite_info["canPeng1"]
		else
			other_chair = self.game_privite_info["canPeng"]
		end
		return self:check_pos(chair_id, other_chair, self.game_all_info.last_out_chair)
	end
	return true
...
function game_sink:check_player_operation_need_wait(chair_id, option_type)
    -- if self.game_player_operation.op and self.game_player_operation.op >= player_op_priority[option_type] then
    --     return true
    -- end
	if option_type == "gang" then
		return not self:check_player_gang_need_wait(chair_id)
    elseif option_type == "bu" then
        return not self:check_player_bu_need_wait(chair_id)
...
local function group_card(pcardinfo, isnotjson, isganghua)
...
--        table.sort(v)
	...
	--小局结算 把胡的牌拿出来独立显示
	if pcardinfo.huCard ~= 0 and not isganghua then
...
function game_sink:check_can_operation_empty()
	...
	if self.game_player_operation.op then
        ...
        chair_id = self.game_player_operation.chair_id
	end
	...
	if self.game_privite_info.canGang and self.game_privite_info.canGang.chair_id ~= 0 and not next(self.game_privite_info.Hu) then
		if player_operation_type < 4 then
			return false
        elseif player_operation_type == 4 then
            if self:check_pos(chair_id, self.game_privite_info.canGang.chair_id,self.game_all_info.last_out_chair) then
                return false
            end
		end
	end
	if self.game_privite_info.canGang1 and self.game_privite_info.canGang1.chair_id ~= 0 and not next(self.game_privite_info.Hu) then
		if player_operation_type < 4 then
			return false
        elseif player_operation_type == 4 then
            if self:check_pos(chair_id, self.game_privite_info.canGang1.chair_id,self.game_all_info.last_out_chair) then
                return false
            end
		end
	end
	if self.game_privite_info.canBu and self.game_privite_info.canBu.chair_id ~= 0 and not next(self.game_privite_info.Hu) then
		if player_operation_type < 3 then
			return false
        elseif player_operation_type == 3 then
            if self:check_pos(chair_id, self.game_privite_info.canBu.chair_id,self.game_all_info.last_out_chair) then
                return false
            end
		end
	end
	if self.game_privite_info.canBu1 and self.game_privite_info.canBu1.chair_id ~= 0 and not next(self.game_privite_info.Hu) then
		if player_operation_type < 3 then
			return false
        elseif player_operation_type == 3 then
            if self:check_pos(chair_id, self.game_privite_info.canBu1.chair_id,self.game_all_info.last_out_chair) then
                return false
            end
		end
	end
	if self.game_privite_info.canPeng and self.game_privite_info.canPeng ~= 0 and not next(self.game_privite_info.Hu) then
		if player_operation_type < 2 then
			return false
        elseif player_operation_type == 2 then
            if self:check_pos(chair_id, self.game_privite_info.canPeng ,self.game_all_info.last_out_chair) then
                return false
            end
		end
	end
	if self.game_privite_info.canPeng1 and self.game_privite_info.canPeng1 ~= 0 and not next(self.game_privite_info.Hu) then
		if player_operation_type < 2 then
			return false
        elseif player_operation_type == 2 then
            if self:check_pos(chair_id, self.game_privite_info.canPeng1 ,self.game_all_info.last_out_chair) then
                return false
            end
		end
	end
	...
	if self.game_privite_info.canChi1 and self.game_privite_info.canChi1 ~= 0 and not next(self.game_privite_info.Hu) then
        if player_operation_type == 0 then
		    return false
        end
	end
...
function game_sink:delete_louHu_limit(chair_id)
	if chair_id == 0 then
		...
		self.louHuChair[chair_id] = 0
        self.louPengChair[chair_id] = {}
	end
end
...
--获得上一个玩家chair_id
function game_sink:get_last_chair(chair_id)
	if chair_id == 1 then
		return #self.players
	else
		return chair_id - 1
...
function game_sink:first_hu_handler(chair_id, iscancel)
	if not self.changsha_first_hu_lock[chair_id] then
...
function game_sink:operationHandler(chair_id, card, card1, istipsClent)
    ...
    local ret, hu_ret = majiang_opration:mo_card(user_card_info, card, chair_id, self.louHuChair, self.draw_count, self.gangLock[chair_id],nil, card1, #self.aftercards)
    ...
    local ret1 
    local hu_ret1
    if self.gangLock[chair_id] and not card1 then
        self.changsha_cache_card = card
    end
    if card1 then
        ret1, hu_ret1 = majiang_opration:mo_card(user_card_info, card1, chair_id, self.louHuChair, self.draw_count, self.gangLock[chair_id], nil, card1, #self.aftercards)
        if self.game_privite_info.Hu_info[chair_id] then
            if not self.game_privite_info.Hu_info1 then
                self.game_privite_info.Hu_info1 = {}
            end 
            self.game_privite_info.Hu_info1[chair_id] = hu_ret1
        else
            self.game_privite_info.Hu_info[chair_id] = hu_ret1
        end
        if not next(ret) then
            ret = ret1
            iscopy = true
        end
        if not next(ret) and not next(ret1) then
            self.changsha_cache_card = 0
	...
	if next(ret) then
        ...
        if iscopy or not ret1 or not next(ret1) then
		    self:send_table_client(chair_id, "game_have_operation", {operation = json.encode(ret)})
        elseif ret1 and next(ret1) then   
            syslog.debug("玩家:["..chair_id.."]可第二次操作:xxxxx"..json.encode(ret1))
            self:count_can_operation(ret1, chair_id, 1, 0, card1, ret1.hucard, true)
            self:send_table_client(chair_id, "game_have_operation", {operation = json.encode(ret), operation1 = json.encode(ret1)})
        end
	else
...
function game_sink:deal_haidi(chair_id)
    ...
    local tab = {}
    tab.canHaidi = true
    syslog.info("player["..chair_id.."]选择海底")
...
--要海底没胡就是他下局庄家
    ...
    local ret, hu_ret = majiang_opration:mo_card(cardInfo, card, chair_id, self.louHuChair, false, false, chair_id)
    table.printR(cardInfo)
    local has_hu = false
    if not next(ret) then
    	has_hu = self:deal_haidi_out_card(chair_id, card)
    else
        has_hu = true
        syslog.debug("玩家:["..chair_id.."]可操作:xxxxx"..json.encode(ret))
		self:count_can_operation(ret, chair_id, 1, 0, card, ret.hucard)
...
function game_sink:deal_haidi_out_card(chair_id, card, iscallback)
	local has_hu = false
	self.haidiType = 3
    for i , k in pairs(self.players) do
        if i ~= chair_id then
            cardInfo = self.players[i].card_info
            local ret1, hu_ret1 = majiang_opration:mo_card(cardInfo, card, i, self.louHuChair, false, false, chair_id)
            if next(ret1) then
    ...
    if iscallback and not has_hu then
    	self:game_end()
    	return false
    end
    return has_hu
...
--    if self.turn ~= chair_id or next(self.game_privite_info.canHu) or next(self.game_privite_info.canGang) or self.game_privite_info.canPeng ~= 0 then
    ...
    --起手胡的时候有大四喜
    if self.game_privite_info.canGang.chair_id == chair_id or self.game_privite_info.canBu.chair_id == chair_id then
        return false, {code = 30005}
    end
    --杠牌后不能换牌
    ...
    --			local ret = majiang_opration:other_out_card(v.card_info.handCards, v.card_info.stackCards,card, k, self.louHuChair)
                local ret, hu_ret = majiang_opration:other_out_card(v.card_info, card, k, self.louHuChair, self.draw_count, self:get_next_chair(chair_id)== k, self.gangLock[k], nil,#self.aftercards, self.louPengChair[k])
                if not self.game_privite_info.Hu_info then
                    self.game_privite_info.Hu_info = {}
                end
        ...
        for k, v in pairs(self.players) do
            if k ~= chair_id then 
                table.printForMajiang(k, v.card_info.handCards)
                local ret, hu_ret = majiang_opration:other_out_card(v.card_info, card, k, self.louHuChair, self.draw_count, self:get_next_chair(chair_id)== k, self.gangLock[k], card1, #self.aftercards, self.louPengChair[k])
                local ret1, hu_ret1 = majiang_opration:other_out_card(v.card_info, card1, k, self.louHuChair, self.draw_count, self:get_next_chair(chair_id)== k, self.gangLock[k], card1, #self.aftercards, self.louPengChair[k])                
                if not self.game_privite_info.Hu_info then
                    self.game_privite_info.Hu_info = {}
                end
                self.game_privite_info.Hu_info[k] = hu_ret
                ...
                if next(ret1) then
                    ...
                    self.game_privite_info.Hu_info1[k] = hu_ret1
                    --确保ret1有值得时候 ret有值
                    if not next(ret) then
                        ret = ret1
                        iscopy = true
			    ...
			    if next(ret) then
                    if iscopy then
				        self:count_can_operation(ret, k, 2, chair_id,card1, ret.hucard)
                    else
                        self:count_can_operation(ret, k, 2, chair_id,card, ret.hucard)
                    end
				    will_wait = true
                    syslog.debug("玩家:["..k.."]可操作:"..json.encode(ret))
                    if iscopy or not next(ret1) then
		                self:send_table_client(k, "game_have_operation", {operation = json.encode(ret)})
                    elseif ret1 and next(ret1) then
                        syslog.debug("玩家:["..k.."]可第二次操作:xxxxx"..json.encode(ret1))
                        self:count_can_operation(ret1, k, 2, chair_id, card1, ret1.hucard, true)
                        self:send_table_client(k, "game_have_operation", {operation = json.encode(ret), operation1 = json.encode(ret1)})
                    end
			    end
            end
        end
...
--	card = self.game_all_info.last_out_card
	...
	if self.game_privite_info.canPeng ~= chair_id and self.game_privite_info.canPeng1 ~= chair_id and not iswait then
	...
	if self:check_player_operation_need_wait(chair_id, "peng") and not iswait then
		syslog.debug("peng need wait")
		local issucc = self:insert_player_operation(chair_id, "peng", card)
        if issucc then
    ...
    self.peng_card_turn[chair_id] = self.peng_card_turn[chair_id] or {}
    table.insert(self.peng_card_turn[chair_id], self.game_all_info.last_out_chair)
	self:delete_louHu_limit(chair_id)
	self.game_all_info.cur_out_chair = chair_id
	self:send_table_client(0,"game_post_timeout_chair", {chair_id = chair_id})
    self:init_player_operation()
    self.turn = chair_id
--    local ret = majiang_opration:check_gang_after(user_card_info, #self.aftercards)
--    if next(ret) then
...
function game_sink:gang_peng_card(chair_id, card, iscallback)
	...
	if self.game_privite_info.canGang.chair_id ~= chair_id and self.game_privite_info.canGang.chair_id ~= chair_id and not iscallback then
		syslog.err("chair_id:["..chair_id.."]杠:["..card.."] 失败")
		return true, {code = -1}
	end
	if self:check_player_operation_need_wait(chair_id, "gang") and not iscallback then
		local issucc = self:insert_player_operation(chair_id, "gang", card, "gang_peng")
        if issucc then
...
function game_sink:gang_mo_card(chair_id, card)
	...
	if self:check_player_operation_need_wait(chair_id, "gang") then
        local issucc = self:insert_player_operation(chair_id, "gang",card,  "gang_mo")
		if issucc then
            self:delete_player_can_operation(chair_id, "canGang")
        else
            self:cancel_action(chair_id, card)
	...
	for k, v in pairs(self.players) do
		if true then
            ...
            if k ~= chair_id and ret.gang_type == 1 then
                local ret, hu_ret = majiang_opration:other_self_gang(v.card_info, card, k, self.louHuChair, true)
				if ret.canHu then
					will_wait = true
					syslog.debug("玩家:["..k.."]可操作:"..json.encode(ret))
...
function game_sink:deal_qianggang_cancel(chair_id, card, isbu)
	--记录玩家杠牌次数
    local user_card_info = self.players[chair_id].card_info
	majiang_opration:handle_mingGang_success(user_card_info.pengCards, user_card_info.gangCards, card)
    self:add_balance_option(chair_id, "mingGang")
...
function game_sink:bu_peng_card(chair_id, card, iscallback)
	...
	if self.game_privite_info.canBu.chair_id ~= chair_id and self.game_privite_info.canBu1.chair_id ~= chair_id and not iscallback then
		syslog.err("chair_id:["..chair_id.."]补张:["..card.."] 失败")
		return true, {code = -1}
	end
	if next(self.game_privite_info.Hu) then
		syslog.err("chair_id:["..chair_id.."]补:["..card.."] 失败,有人胡牌，自己不胡而补")
		--有人胡  去碰相当于取消（只有在别人胡 你也胡还有碰， 点碰会进这里）
		self:cancel_action(chair_id, card)
		return true, {code = 30002}
	end
	if self:check_player_operation_need_wait(chair_id, "bu") and not iscallback then
		local issucc = self:insert_player_operation(chair_id, "bu", card, "bu_peng")
        if issucc then
...
function game_sink:bu_mo_card(chair_id, card)
	...
	if self:check_player_operation_need_wait(chair_id, "bu") then
        local issucc = self:insert_player_operation(chair_id, "bu",card,  "bu_mo")
		if issucc then
		    self:delete_player_can_operation(chair_id, "canBu")
        else
            self:cancel_action(chair_id)
	...
	for k, v in pairs(self.players) do
		...
		self:send_table_client(k, "game_bu_card", param)
        if k ~= chair_id and ret.gang_type == 1 then
            local ret, hu_ret = majiang_opration:other_self_bu(v.card_info, card, k, self.louHuChair)
			if ret.canHu then
				will_wait = true
				syslog.debug("玩家:["..k.."]可操作:"..json.encode(ret))
				self:count_can_operation(ret, k, 3, chair_id, nil, card)
...
function game_sink:bu_card(chair_id, card)
	...
	if self.game_privite_info.canBu.chair_id ~= chair_id and self.game_privite_info.canBu1.chair_id ~= chair_id then
		return true, {code = 30002}
	end
    if next(self.game_privite_info.Hu) then
        self:cancel_action(chair_id)
...
function game_sink:chi_card(chair_id, card_table, iscallBack)
	...
	-- if self.game_privite_info.canChi ~= chair_id and self.game_privite_info.canChi1 ~= chair_id and not iscallBack then
 ...
 --        table.printR(card_table)
	...
	if not iscallBack then
		local is_ok = false
		if self.game_privite_info.canChi == chair_id or self.game_privite_info.canChi1 == chair_id then
			if user_card_info.stackCards[card_table.will_chi_card[1]] and user_card_info.stackCards[card_table.will_chi_card[2]] then
				local tab = {}
				table.insert(tab, card_table.will_chi_card[1])
				table.insert(tab, card_table.will_chi_card[2])
				table.insert(tab, card_table.chi_card)
				table.sort(tab)
				if tab[1] + 1 ==  tab[2] and tab[2] +1 == tab[3] then
					local t = math.floor(tab[1]/10)
					local v = tab[1]%10
					if t > 0 and t < 4 and v > 0 and v < 8 then
						is_ok = true
					end
				end
			end
	...
	if next(self.game_privite_info.Hu) then
		syslog.err("chair_id:["..chair_id.."]吃:[] 失败,有人胡牌，自己不胡而碰")
		--有人胡  去吃相当于取消（只有在别人胡 你也胡还有吃， 点吃会进这里）
		self:cancel_action(chair_id, card)
		return true, {code = 30002}
	end
    if self:check_player_operation_need_wait(chair_id, "chi") and not iscallBack then
        local issucc = self:insert_player_operation(chair_id, "chi", card_table)
		if issucc then
...
function game_sink:deal_gang_balance(gang_chair, lose_chair, gangType)
	...
	if lose_chair == 0 then
		for k, v in pairs(self.players) do
			if k ~= gang_chair then
				--明杠每人1分
				if gangType == 1 then
					v.changsha_balance_info.getpoint = v.changsha_balance_info.getpoint -1
					win_point = win_point + 1
					GangType = "mingGang"
				--暗杠每人2分
				else
					v.changsha_balance_info.getpoint = v.changsha_balance_info.getpoint -2
		...
		if self.table_config.player_count == 2 then
			--两人玩法点杠1分
			local lose_player_balance_info = self.players[lose_chair].changsha_balance_info
			lose_player_balance_info.getpoint = lose_player_balance_info.getpoint -1
			win_point = win_point + 1
			self:add_gangType(lose_player_balance_info.gangType, "fangGang")
		else
			--点杠3分
			local lose_player_balance_info = self.players[lose_chair].changsha_balance_info
			lose_player_balance_info.getpoint = lose_player_balance_info.getpoint -3
	...
	--杠的玩家获得的积分
	local win_player_balance_info = self.players[gang_chair].changsha_balance_info
	win_player_balance_info.getpoint = win_player_balance_info.getpoint + win_point
...
local zimo_point_table = {
    ["shuang_hao_seven_hu"] = 9,
    ["hao_seven_hu"] = 6,
    ["seven_hu"] = 3,
    ["pengpenghu"] = 3,
}

local dianpao_point_table = {
...
function game_sink:deal_hu_card(hu_type, info, chair_id, chair_id1)
    ...
    local hu_count = 0
    local chengbao_point = 0
    local chengbao_chair = 0
    if hu_type == 1 then
    ...
    elseif hu_type == 2 then
        ...
        if info.pinghu then
            ...
            if chair_id1 then
                point = bigdianpao*hu_count
            else
                point = bigzimo*hu_count
            end
        end
        --门清加分
        if info.menqing then
            point = point + 1
        end
        --抢杠胡
        if info.qiangganghu then
            point = 6
            hucount = 1
            info = {["qiangganghu"]= 1}
        end
        --杠上跑
        if info.gangpao then
            point = 9
            hucount = 1
            info = {["gangpao"]= 1}
        end
        --海底炮
        if info.haidipao then
            point = 9
            hucount = 1
            info = {["haidipao"]= 1}
        end
        ...
        local user_card_info = self.players[chair_id].card_info
        --清一色
        if info.qingyise then
            local chi_num = #user_card_info.chiCards 
            chi_chair = self:get_last_chair(chair_id)
            local tab = {}
            tab[chi_chair] = chi_num
            if chi_num >= 3 then
                if not chair_id1 then
                    chengbao_chair = chi_chair
                    self.players[chengbao_chair].changsha_balance_info.hu_info.chengbao = 1
                elseif chair_id1 then
                    local dianpao_user_card_info = self.players[chair_id1].card_info
                    if majiang_opration:check_ting_card(dianpao_user_card_info.handCards, dianpao_user_card_info) then
                        chengbao_chair = chi_chair
                        self.players[chengbao_chair].changsha_balance_info.hu_info.chengbao = 1
                    end
                end
                if chair_id1 and chengbao_chair ~= 0 then
                    chengbao_point = 6
                else
                    chengbao_point = 3
                end
            else
                if self.peng_card_turn[chair_id] then
                    for i= 1, #self.peng_card_turn[chair_id] do
                        tab[self.peng_card_turn[chair_id][i]] = tab[self.peng_card_turn[chair_id][i]] or 0
                        tab[self.peng_card_turn[chair_id][i]] = tab[self.peng_card_turn[chair_id][i]] + 1
                        if tab[self.peng_card_turn[chair_id][i]] >= 3 then
                            if not chair_id1 then
                                chengbao_chair = self.peng_card_turn[chair_id][i]
                                self.players[chengbao_chair].changsha_balance_info.hu_info.chengbao = 1
                            elseif chair_id1 then
                                local dianpao_user_card_info = self.players[chair_id1].card_info
                                if majiang_opration:check_ting_card(dianpao_user_card_info.handCards, dianpao_user_card_info) then
        ...
        if info.jiangjianghu then
            local tab = {}
            if self.peng_card_turn[chair_id] then
                for i= 1, #self.peng_card_turn[chair_id] do
                    tab[self.peng_card_turn[chair_id][i]] = tab[self.peng_card_turn[chair_id][i]] or 0
                    tab[self.peng_card_turn[chair_id][i]] = tab[self.peng_card_turn[chair_id][i]] + 1
                    if tab[self.peng_card_turn[chair_id][i]] >= 2 then
                        if not chair_id1 then
                            chengbao_chair = self.peng_card_turn[chair_id][i]
                            self.players[chengbao_chair].changsha_balance_info.hu_info.chengbao = 1
                        else
            ...
            if self.peng_card_turn[chair_id] then
                for i= 1, #self.peng_card_turn[chair_id] do
                    ...
                    if tab[self.peng_card_turn[chair_id][i]] >= 3 then
                        ...
                        self.players[chengbao_chair].changsha_balance_info.hu_info.chengbao = 1
                        if chair_id1 then
                            chengbao_point = 6 + chengbao_point
                        else
                            --会出现碰碰胡又将将胡
                            chengbao_point = 3 + chengbao_point
                        end
                        break
                    end
                end
        ...
        --庄闲分
        local zhuangxian_point = hucount
        if zhuangxian_point < 1 then
            zhuangxian_point = 1
        elseif zhuangxian_point > 3 then
            zhuangxian_point = 3
        end
        local player2 = self.players[chengbao_chair]
        if chair_id1 then
            player1 = self.players[chair_id1]
            local bank_point = 0
            if self.game_config.idle and (self:get_banker_chair() == chair_id1 or self:get_banker_chair() == chair_id) then
                bank_point = zhuangxian_point
            end
            if chengbao_chair == 0 then
                ...
                if self.game_config.yibaodaodi or chengbao_chair == chair_id1 then
                    player2.changsha_balance_info.getpoint = player2.changsha_balance_info.getpoint - point - bank_point
                    player.changsha_balance_info.getpoint = player.changsha_balance_info.getpoint + point + bank_point
                else
                    player.changsha_balance_info.getpoint = player.changsha_balance_info.getpoint + point + bank_point
                    player2.changsha_balance_info.getpoint = player2.changsha_balance_info.getpoint - chengbao_point
                    player1.changsha_balance_info.getpoint = player1.changsha_balance_info.getpoint - point - bank_point + chengbao_point
                end
            end
        else
            local tmpPoint = 0
            if chengbao_chair == 0 or chengbao_chair == chair_id1 then
                for i , k in pairs(self.players) do
                    if i ~= chair_id then
                        if self.game_config.idle and (self:get_banker_chair() == i or self:get_banker_chair() == chair_id) then
                            k.changsha_balance_info.getpoint = k.changsha_balance_info.getpoint - point - zhuangxian_point
                        else
                            k.changsha_balance_info.getpoint = k.changsha_balance_info.getpoint - point 
                        end
                    end
                ...
                if self.game_config.yibaodaodi and chengbao_chair ~= 0 then
                   local chengbao_chair_info = self.players[chengbao_chair].changsha_balance_info
                   for i , k in pairs(self.players) do
                        if i ~= chair_id then
                            if self.game_config.idle and (self:get_banker_chair() == i or self:get_banker_chair() == chair_id) then
                                chengbao_chair_info.getpoint = chengbao_chair_info.getpoint - point - zhuangxian_point 
                            else
                                chengbao_chair_info.getpoint = chengbao_chair_info.getpoint - point 
                            end
                        end
                    end
                else
                    for i , k in pairs(self.players) do
                        if i ~= chair_id then
                            if i == chengbao_chair then
                                if self.game_config.idle and (self:get_banker_chair() == i or self:get_banker_chair() == chair_id) then
                                    k.changsha_balance_info.getpoint = k.changsha_balance_info.getpoint - point - zhuangxian_point - (#self.players -2)*chengbao_point
                                else
                                    k.changsha_balance_info.getpoint = k.changsha_balance_info.getpoint - point - (#self.players -2)*chengbao_point
                                end
                            else
                                if self.game_config.idle and (self:get_banker_chair() == i or self:get_banker_chair() == chair_id) then
                                    k.changsha_balance_info.getpoint = k.changsha_balance_info.getpoint - point - zhuangxian_point + chengbao_point
                                else
                                    k.changsha_balance_info.getpoint = k.changsha_balance_info.getpoint - point + chengbao_point
                                end
                            end
                        end
                    end
            ...
            if self.game_config.idle then
                if self:get_banker_chair() == chair_id then
                    player.changsha_balance_info.getpoint = player.changsha_balance_info.getpoint + point*Count + Count*zhuangxian_point
                else
                    player.changsha_balance_info.getpoint = player.changsha_balance_info.getpoint + point*Count + zhuangxian_point
                end
            else
                player.changsha_balance_info.getpoint = player.changsha_balance_info.getpoint + point*Count 
            end
        end
        for i , k in pairs(info) do
            if player.changsha_balance_info.hu_info[i] then
                player.changsha_balance_info.hu_info[i] = player.changsha_balance_info.hu_info[i] + 1
            else
                player.changsha_balance_info.hu_info[i] = 1
            end
        end
...
function game_sink:hu_card(chair_id, card)
	if not card then
		syslog.err("no hu card !!!")
		return false, {code = 30001}
	end
	syslog.info("chair_id:["..chair_id.."]胡"..card)
	if not self.is_playing then
		return false, {code = 30001}
	end
	--处理多胡的问题
	...
	for k, v in pairs(self.game_privite_info.canHu) do
		--结算
		if v.chair_id == chair_id then
            if card == v.card or self.game_config.gangkaiquanzhong then
			    self.game_record:record_game_action(chair_id, 6, v.card)
			    self.is_Hu = true
			    huIndex = k
                local huinfo 
                if self.game_privite_info.Hu_info[chair_id].hucard == v.card then
                    huinfo = self.game_privite_info.Hu_info[chair_id]
                else
                    huinfo = self.game_privite_info.Hu_info1[chair_id]
                end
			    --胡的牌
                if self.players[chair_id].card_info.huCard == 0 then
			        self.players[chair_id].card_info.huCard = v.card
                else
                    self.players[chair_id].card_info.huCard2 = v.card
                end
			    --接炮 or 抢杠胡 or 海底捞 把胡的牌加到手牌里
			    if v.lose_chair ~= 0 or huinfo.haidilao then
			    ...
			    if v.lose_chair ~= 0 then
                    ...
                    self:deal_hu_card(2, huinfo, chair_id, v.lose_chair)
                    if v.op == 3 then
                        hutype = 3
                    else
                        hutype = 2
			    ...
			    elseif v.lose_chair == 0 then
                    self:deal_hu_card(2, huinfo, chair_id)
                    hutype = 1
			    end
                --只算一个胡（会出现2张牌一样的情况）
                if not self.game_config.gangkaiquanzhong then
                    break
                end
            end
		end
	...
	if huIndex ~= 0 then
        ...
        --取除改玩家别的胡信息
        for i , k in pairs(self.game_privite_info.canHu) do
            if k.chair_id == chair_id then
                table.remove(self.game_privite_info.canHu, i)
                break
            end
        end
...
local function get_fan_point(hu_info)
    local fan = 0
    if not hu_info then
        return 1
    end
    hu_info["hucard"] = nil
    if not next(hu_info) then
        return 1
    end
    for i, k in pairs(hu_info) do
        if i == "shuang_hao_seven_hu" then
            fan = fan + 6
        elseif i == "hao_seven_hu" then
            fan = fan + 4
...
function game_sink:cancel_action(chair_id)
    ...
    --不要海底
    if self.onHaidi == chair_id and self.haidiType == 1 then
        local id = chair_id + 1 
	...
	for k, v in  pairs(self.game_privite_info.canHu) do
		if v.chair_id == chair_id then
            ...
            self.louHuChair[chair_id] = self.louHuChair[chair_id] or 0
            local fan_tmp = get_fan_point(self.game_privite_info.Hu_info[chair_id])
            if fan_tmp > self.louHuChair[chair_id] then
                self.louHuChair[chair_id] = fan_tmp
            end
            fan_tmp = get_fan_point(self.game_privite_info.Hu_info1[chair_id])
            if fan_tmp > self.louHuChair[chair_id] then
                self.louHuChair[chair_id] = fan_tmp
            end
            hu_card = v.card
    ...
    if self.game_privite_info.canPeng == chair_id then
        self.louPengChair[chair_id] = self.louPengChair[chair_id] or {}
        self.louPengChair[chair_id][self.game_privite_info.pengcard] = true
        can_cancel = true
    end
    if self.game_privite_info.canPeng1 == chair_id then
        self.louPengChair[chair_id] = self.louPengChair[chair_id] or {}
        self.louPengChair[chair_id][self.game_privite_info.pengcard1] = true
        can_cancel = true
    end
    --是否有操作取消
    if self.game_privite_info.canPeng ~= chair_id and self.game_privite_info.canPeng1 ~= chair_id and not can_cancel then
        if self.game_privite_info.canGang.chair_id ~= chair_id and self.game_privite_info.canGang1.chair_id ~= chair_id then
            if self.game_privite_info.canBu.chair_id ~= chair_id and self.game_privite_info.canBu1.chair_id ~= chair_id then
                if self.game_privite_info.canChi ~= chair_id and self.game_privite_info.canChi1 ~= chair_id then
                    return false, {code = 30002}
                end
            end
        end
	...
	if self:check_can_operation_empty() and self.isqianggang == 0 then
		if not self.is_Hu then
			if not self:deal_player_operation(chair_id) then
    ...
    elseif self.isqianggang ~= 0 and not next(self.game_privite_info.canHu) then
    	...
    	if self.qianggang_card ~= 0 then
	        self:deal_qianggang_cancel(self.isqianggang, self.qianggang_card)
	        self.isqianggang = 0
	        self.qianggang_card = 0
	    elseif self.qiangbu_card ~= 0 then
	        self:deal_qianggang_cancel(self.isqianggang, self.qiangbu_card, true)
	        self.qiangbu_card = 0
	        self.qiangbu_card = 0
	    end
	end
...
--    self.next_banker_chair
    ...
    if iszimo then
        ...
        for i , k in pairs(self.players) do
            if i ~= self.next_banker_chair then 
                local space1 = i - self.next_banker_chair + 1
                if space1 <= 0 then
        ...
        if self.dianPao == 1 then
            ...
            space = self.changsha_lose_chair_id - self.next_banker_chair + 1
            if space <= 0 then
        ...
        for i, k in pairs(self.changsha_hu_chair_id) do
            ...
            local space1 = k - self.next_banker_chair + 1
            if space1 <= 0 then
                space1 = space1 + #self.players
            end
            local birdtab1 = birdTmp[space1] or {}
...
--    end
    ...
    for k , v in pairs(self.players) do 
        ...
        local isganghua = false
        if next(v.changsha_balance_info.hu_info) then
            --local tab = json.decode(v.changsha_balance_info.hu_info)
            if v.changsha_balance_info.hu_info["ganghua"] then
                isganghua = true
            end
        end
        tmp.handCard = group_card(v.card_info, false, isganghua)
        tmp.huCard = v.card_info.huCard
        tmp.huCard2 = v.card_info.huCard2
        tmp.first_hu = v.changsha_balance_info.first_hu
        tmp.hu_info = json.encode(v.changsha_balance_info.hu_info)
        tmp.huType = v.changsha_balance_info.huType
        tmp.point = self:add_balance_point(k ,tmp.getpoint) + 1000
        tmp.gangType = json.encode(v.changsha_balance_info.gangType)
        table.insert(self.changsha_game_end_balance_info.changsha_player_balance, tmp)
    end
...
function game_sink:post_game_reconnect(chair_id)
	...
	for k, v in pairs(self.players) do
        ...
        if k == chair_id then
            if next(self.changsha_first_hu_lock) and chair_id == self:get_banker_chair() then
...
--                    if not ret.canOperation[k] then
                      if card == 0 or card == hv.card then
                        ret.canOperation[k] = true
                        ret.canOperation.hutype = hv.op
                        ret.canOperation.hucard = hv.card
                        card = hv.card
                    else
                        ret.canOperation1[k] = true
                        ret.canOperation1.hutype = hv.op
                        ret.canOperation1.hucard = hv.card
                        card1 = hv.card
                    end
				end
			end
		elseif k == "canGang" or k == "canGang1" then
			if v.chair_id == chair_id then
                if card == 0 or card == v.card[1] or card == v.card[2] or card == v.card[3] then
                    ret.canOperation["canGang"] = v.card
                    card = v.card[1]
                else
                    ret.canOperation1["canGang"] = v.card
                    card1 = v.card[1]
                end
			end
        elseif k == "canBu" or k == "canBu1" then
            if v.chair_id == chair_id then
                if card == 0 or card == v.card[1] or card == v.card[2] or card == v.card[3] then
                    ret.canOperation["canBu"] = v.card
                    card = v.card[1]
                else
                    ret.canOperation1["canBu"] = v.card
                    card1 = v.card[1]
                end
            end
		elseif k == "canPeng" or k == "canPeng1" then
			if v == chair_id then
                if k == "canPeng" then
                    if card == 0 or card == self.game_privite_info.pengcard then
                        ret.canOperation.canPeng = self.game_privite_info.pengcard
                        card = ret.canOperation.canPeng
                    else
                        ret.canOperation1.canPeng = self.game_privite_info.pengcard
                        card1 = ret.canOperation1.canPeng
                    end 
                else
                    if card == 0 or card == self.game_privite_info.pengcard1 then
                        ret.canOperation.canPeng = self.game_privite_info.pengcard1
                        card = ret.canOperation.canPeng
                    else
                        ret.canOperation1.canPeng = self.game_privite_info.pengcard1
                        card1 = ret.canOperation1.canPeng
                    end
                end
			end
        elseif k == "canChi" or "canChi1" then
            if v == chair_id then
                if k == "canChi" then
                    if card == self.game_privite_info.Chi_info[1] or card == 0 then
                        ret.canOperation.canChi = self.game_privite_info.Chi_info
                        card = ret.canOperation.canChi[1]
                    else
                        ret.canOperation1.canChi = self.game_privite_info.Chi_info
                        card1 = ret.canOperation1.canChi[1]
                    end
                else
                    if card == self.game_privite_info.Chi_info1[1] or card == 0 then
                        ret.canOperation.canChi = self.game_privite_info.Chi_info1
                        card = ret.canOperation.canChi[1]
                    else
                        ret.canOperation1.canChi = self.game_privite_info.Chi_info1
                        card1 = ret.canOperation1.canChi[1]
                    end
                end
            end
    ...
    if next(ret.canOperation1) then
        ret.canOperation1 = json.encode(ret.canOperation1)
    else
        ret.canOperation1 = nil
    end
    syslog.info(ret.canOperation)
    if ret.canOperation1 then
        syslog.info(ret.canOperation1)
    end
