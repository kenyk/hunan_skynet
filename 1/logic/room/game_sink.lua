...
function game_sink:deal_cards()
    ...
    --扎马  一马全中是1
    if self.game_config.hongzhong and self.game_config.find_bird > 1 then
...
function game_sink:deal_hu_balance(win_chair, lose_chair, pbirdPoint, op)
	...
	elseif lose_chair ~= 0 then
		...
		elseif op == 3 then
			...
			local game_balance_info = self.players[lose_chair].balance_info
			game_balance_info.huPoint = game_balance_info.huPoint + win_point
		...
		if v.chair_id == chair_id then
            ...
            if self.game_config.hongzhong then
                if not next(self.game_end_balance_info.birdCard) then
                    if self.game_config.find_bird ~= 1 then
                        ...
                        if (v.lose_chair == 0 and checkHashongzhong(self.players[chair_id].card_info.handCards) and self.aftercards[1]) or self.game_config.find_bird == 1 then
                        ...
                        if self.aftercards[1] == 45 then
                            point = 10
                            table.insert(self.game_end_balance_info.birdCard, self.aftercards[1])
                            table.insert(self.game_end_balance_info.zhongbird, self.aftercards[1])
                        elseif self.aftercards[1] then
                            point = math.floor(self.aftercards[1]%10)
                            table.insert(self.game_end_balance_info.birdCard, self.aftercards[1])
                            table.insert(self.game_end_balance_info.zhongbird, self.aftercards[1])
                        end
                        self.game_end_balance_info.birdNum = point
                    end
                end
            else
			    if not next(self.game_end_balance_info.birdCard) then
